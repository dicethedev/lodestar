...
import {BeaconStateView, CachedBeaconStateAltair, Index2PubkeyCache} from "@lodestar/state-transition";
...
describe("opPool", () => {
  ...
  bench({
    ...
    beforeEach: () => {
      ...
      fillBlsToExecutionChanges(beaconState.epochCtx.index2pubkey, pool, beaconState, MAX_BLS_TO_EXECUTION_CHANGES);
  ...
  bench({
    ...
    beforeEach: () => {
      ...
      // TODO: feed index2pubkey separately instead of getting from originalState
      fillBlsToExecutionChanges(beaconState.epochCtx.index2pubkey, pool, beaconState, maxItemsInPool);
